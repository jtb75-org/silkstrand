...
interface DataTableProps<T> {
   ...
   * break selection once a column is sorted; ADR 018/020 selection contract).
   */
  getRowId?: (row: T) => string;
  /** Row click → navigation. Adds `.clickable-row` + keyboard activation. */
  onRowClick?: (row: T) => void;
  // Opt-in selection. The PAGE owns the selected-id Set; DataTable just renders
  // checkboxes from these props and stays a thin wrapper (ADR 018 D5).
  selectable?: boolean;
...
// DataTable — thin styled wrapper over @tanstack/react-table (ADR 018 D5,
// ported to the backoffice per ADR 020). Pages depend on THIS component, not
// the library, so the table engine stays swappable. It renders the existing
// `.table` design-system styles and adds click-to-sort headers (Lucide
// indicators), opt-in row selection, and clickable rows. Sorting/filtering
// behavior is the library's; the markup and styling are ours.
