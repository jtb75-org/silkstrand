...
func (s *PostgresStore) UpsertBundle(ctx context.Context, b model.Bundle) (*model.Bundle, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`INSERT INTO bundles (id, tenant_id, name, version, framework, target_type, engine, control_count, gcs_path, signature, tarball_hash)
		 VALUES (COALESCE(NULLIF($1::text, '')::uuid, uuid_generate_v4()), NULLIF($2::text, '')::uuid, $3, $4, $5, $6, $7, $8, $9, $10, $11)
...
// ListHTTPServiceHostnames returns the tenant's imported http_service hostnames
// (ADR 014 D4) — the dispatch targets for a dns_list-scoped scan definition.
func (s *PostgresStore) ListHTTPServiceHostnames(ctx context.Context, tenantID string) ([]string, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT hostname FROM assets
		 WHERE tenant_id = $1 AND resource_type = 'http_service' AND hostname IS NOT NULL
