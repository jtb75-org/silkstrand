...
| **ADR 013** | Guided agent rollout (Deploy → Discover → Prove): `--allow-cidr` allowlist seeding (#353), `agent_allowlist` scan scope (#354), auto-discover on connect (#355), agent zones + install-time overlap preview/modal (#356), agent egress proxy + custom CA via centralized `nettls` transport (#357), docker-mode proxy/CA parity (#360). ADR Accepted. | ✅ shipped |
